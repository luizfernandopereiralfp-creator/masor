...
**Ponto que o mercado esconde:** a transmissão ao eSocial **não é uma trilha paga**. A Receita/MTE publicam a especificação livremente e qualquer software com certificado ICP-Brasil A1 ou A3 transmite direto, de graça. ○ **CORRIGIDO EM 30/08/2026 — A AFIRMAÇÃO ABAIXO ESTAVA ERRADA.** O Manual de Orientação do Desenvolvedor v1.15 foi baixado e lido: **não existe API REST oficial do eSocial**. As 125 páginas não contêm uma única menção a `REST`, `JSON` ou `Bearer`, e todos os endpoints são SOAP. O documento `02-esocial-integracao-tecnica.md` estava certo. Ver `fontes/VERIFICACAO.md`. O texto original fica registrado abaixo, riscado, para não apagar o histórico do erro:

~~Este documento afirmou existir uma **API REST oficial**, assíncrona, JSON com Bearer token e lote de até 50 eventos, com base na [especificação de recepção de lote no portal](https://www.gov.br/esocial/pt-br/documentacao-tecnica/manuais/4915736_especificacao_recepcaolote_empresas.pdf) — mas o documento `02-esocial-integracao-tecnica.md` conclui o oposto (P-01: nenhum canal REST oficial localizado). **Nenhum dos dois conseguiu abrir a especificação**, porque o egress para `gov.br` estava bloqueado. Isso muda a arquitetura do transporte e o esforço de construção: resolver antes de decidir. Ver `FONTES-A-BAIXAR.md`, item A5. O que você paga em (a) é: manutenção de leiaute, assinatura XMLDSig, fila/retentativa/recibo, tradução de erro e alguém para ligar às 23h do dia 15. É valor real — mas é uma camada fina, e é a mais fácil de trocar de fornecedor.
